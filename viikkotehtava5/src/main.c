/*
 * Tehtävän tekijät:
 * Salla-Mari Rokkonen ja Antti Ojala
 * 1p (Perusosa): Toteutettu time_parse-funktio Google Test -testeillä.
 * Parseri on integroitu RTOS-koodiin käyttäen k_timer-ajastinta (Hardware/Software timer),
 * joka sytyttää punaisen ledin annetun ajan (esim. 000005) kuluttua.
 *
 * +1p (Lisätestit): time_parse-funktioon ja testeihin lisätty kattavat
 * rajatarkistukset: tyhjä osoitin (NULL), tasan 6 merkin pituus, pelkät
 * numerot (isdigit) sekä 0 sekunnin ajastuksen esto.
 *
 * +1p (Liikennevalojen testaus): Luotu uusi parseri (sequence_parse) ja sille
 * oma testisuite, joka validoi UART-liikennevalokomennot (esim. R,1000).
 * Parseri hylkää väärät kirjaimet ja muotoilut, ja se on integroitu RTOS-koodin
 * dispatcher_taskiin suojaamaan ohjelmaa virheellisiltä syötteiltä.
 */
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3) {
    ...
    while (true) {
        ...
        if (strlen(sequence) == 6 && isdigit ((unsigned char)sequence[0])) {
