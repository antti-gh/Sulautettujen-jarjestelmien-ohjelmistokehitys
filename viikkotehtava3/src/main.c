...
//Tehtävässä on jatkettu edellisen tehtävän koodia, jonka vuoksi koodissa on vielä nappien toimintoja.
//Tehtävä aloitettiin hoitamalla viestinvälitys k_fifo-puskureilla UART->dispatcher->ledit.
//Tehtävässä poistettiin superloopit eli ne toimivat nyt single-shot taskeina.
//Lisättiin Thread API:n käyttö k_thread_create - komennolla.
//Ohjelma tallentaa saapuneet värit ja ajat taulukoihin ja toistaa ne T komennolla.
...
void button_2_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    printk("Button 2 pressed (Yellow manual)\n");
}
 
void button_3_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    printk("Button 3 pressed (Green manual)\n");
}
 
void button_4_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    printk("Button 4 pressed (Yellow blink mode)\n");
}
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3) {
    ...
    while (true) {
        ...
        if (color == 'R') {
            printk("Dispatcher:punainen valo, aika: %d\n", time);
            ...
            printk("Dispatcher:keltainen valo, aika: %d\n", time);
           ...
           // k_condvar_signal(&yellow_condvar);
            ...
            printk("Dispatcher:vihreä valo, aika: %d\n", time);
...
void red_led_task(void *, void *, void*) {
    ...
    //while (true) {
        //odotetaan dispatcherin herätystä
        //k_mutex_lock(&led_mutex, K_FOREVER);
