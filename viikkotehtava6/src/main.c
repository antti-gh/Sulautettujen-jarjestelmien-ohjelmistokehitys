/*Tekijät Salla-Mari Rokkonen ja Antti Ojala
 * 1p Perusosa (Aikamerkkijono robotista):
 *    Ohjelma ottaa UARTin kautta vastaan aikamerkkijonoja, käsittelee ne ja palauttaa
 *    vastauksen Robotille. Testattu on sekä oikea aika (000120 -> 80) että
 *    virheellinen aika (006700 -> virhekoodi -3).
 *
 * +1p Lisäosa (Lisätään aikamerkkijonon testikeissejä):
 *    Robot-skriptiin on lisätty ylimääräisiä yksikkötestejä aikaparserille. Näitä ovat
 *    liian lyhyt merkkijono (123 -> virhekoodi -1) ja nolla-aika (000000 -> virhekoodi -4).
 *
 * +1p Lisäosa (Ajastetun sekvenssin testikeissit):
 *    Robotilla testataan myös sekvenssiparserin virheenkäsittelyä. Testattu on
 *    oikea komento (R,100 -> 0), väärä väri (K,100 -> virhekoodi -11) ja
 *    väärä aika (R,0 -> virhekoodi -13).
 */
...
// Ajastimen sammutuskeskeytys (sammuttaa valon)
void timer_off_cb(struct k_timer *timer_id) {
    gpio_pin_set_dt(&red, 0);
    printk("Red LED off\n");
}
K_TIMER_DEFINE(my_timer_off, timer_off_cb, NULL);
 
//Ajastinkeskeytys, joka tapahtuu kun annettu HHMMSS-aika on kulunut
void timer_on_cb(struct k_timer *timer_id) {
    gpio_pin_set_dt(&red, 1);
    printk("Red LED on\n");
    //Käynnistetään ajastin, joka sammuttaa valon 2 sekunnin kuluttua
    k_timer_start(&my_timer_off, K_SECONDS(2), K_NO_WAIT);
}
K_TIMER_DEFINE(my_timer, timer_on_cb, NULL);
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3) {
    ...
    while (true) {
        ...
        //Jos viestin pituus on 6 ja ensimmäinen merkki on numero, se on aikakoodi
        if (isdigit((unsigned char)sequence[0])) {
            int parsed_seconds = time_parse(sequence);

			printk("%dX", parsed_seconds);
 
            if (parsed_seconds > 0) {
                debug_printf("Dispatcher: Ajastin asetettu %d sekunniksi\n", parsed_seconds);
                k_timer_start(&my_timer, K_SECONDS(parsed_seconds), K_NO_WAIT);
            } else {
                debug_printf("Dispatcher:Virheellinen aikamerkkijono (virhekoodi %d)\n", parsed_seconds);
            }
            continue; //Hypätään while silmukan alkuun käsittelemättä RYG komentoa
        }
        // Tarkistetaan, onko kyseessä oikeanlainen RYG-komento
        int seq_status = sequence_parse(sequence);
		// Lisäys robottia varten
		printk("%dX", seq_status);
        if (seq_status != SEQ_OK) {
            debug_printf("Dispatcher: Virheellinen komento (Virhekoodi: %d)\n", seq_status);
            continue;
        }
...
int time_parse(char *time) {
    ...
    //Tarkistetaan, ettei aika ole nolla sekuntia
    if (seconds == 0) {
